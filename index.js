...
app.get('/getProject', (req, res) => {
    ...
    con.query(sql, (err, result) => {
        if (err) {
            return res.json({ Error: "Get project error in sql" });
        }
        return res.json({ Status: "Success", Result: result });
    });
});
...
app.post('/createproject', (req, res) => {
    ...
    con.query("INSERT INTO nodejsdb.projects (project_id, project_name) VALUES (?, ?)", [project_id, project_name],
        (err, result) => {
            if (result) {
                res.send(result);
            } else {
...
app.get('/getdata', (req, res) => {
    const sql = `
    SELECT mw.date, mw.employeename, mw.total_duration, p.project_name
    FROM janu.managework mw 
    JOIN nodejsdb.projects p ON mw.topic_project = p.project_name
  `;
    con.query(sql, (err, result) => {
        if (err) {
            console.error("SQL query error:", err);
            return res.json({ Error: "Get project error in SQL", Details: err.message });
        }
        return res.json({ Status: "Success", Result: result });
    });
});
...
app.post('/create1', (req, res) => {
    ...
    const topic_project = req.body.topic_project;
    const description = req.body.description;
    const total_duration = req.body.total_duration;

    if (!employeename || !date || !topic_project || !total_duration || !description) {
        return res.status(400).json({ message: 'All fields are required' });
    }

    con1.query("INSERT INTO managework (date, employeename, topic_project, description, total_duration) VALUES (?, ?, ?, ?, ?)", [date, employeename, topic_project, description, total_duration],
        (err, result) => {
            if (err) {
